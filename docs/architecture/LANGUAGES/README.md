...
```
docs/architecture/
├── SKILL.md                    ← Skill entry point (6 steps)
...
├── TEMPLATES/                  ← Output templates (generic — no project data)
│   ├── architecture.html       # Interactive HTML viewer (contains {{SYSTEM_DATA}} placeholder)
│   └── system-data.json        # JSON data schema template
└── SCRIPTS/
...
|------|--------|---------|
| `docs/architecture/architecture.html` | Interactive HTML | Browser viewing, presentations |
| `docs/architecture/system-data.json` | JSON | CI pipelines, automation, further analysis |

**Build rule:** `architecture.html` is generated by replacing `{{SYSTEM_DATA}}` in `TEMPLATES/architecture.html` with the actual JSON from `system-data.json`. The template itself stays generic.
